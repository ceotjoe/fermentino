	...
	receiveControlSettings(function(){
		switch(controlSettings.mode){
			case 'p':
				...
				statusMessage("normal","Running in leaven profile mode");
				break;
			case 'b':
				$('#control-panel').tabs( "select" , 1);
				statusMessage("normal","Running in leaven constant mode");
				break;
			case 'o':
...
function applySettings(){
	...
	if($("#profile-control").hasClass('ui-tabs-hide') === false){
		...
		statusMessage("highlight","Mode set to leaven profile");
	}
		...
		statusMessage("highlight","Mode set to leaven constant");
	}
